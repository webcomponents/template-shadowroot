...
/*
 * Traverses the DOM to find all <template> elements with a `shadowroot` or
 * `shadowrootmode` attribute and move their content into a ShadowRoot on
...
export const hydrateShadowRoots = (root: ParentNode) => {
  ...
  // The outer loop traverses down, looking for <template shadowroot> or
  // <template shadowrootmode> elements. The inner loop traverses back up,
  // hydrating them in a postorder traversal.
